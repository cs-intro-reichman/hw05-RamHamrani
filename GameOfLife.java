...
public class GameOfLife {
	...
	private static void test2(String fileName) {
		int[][] board = read(fileName);
		System.out.println("Count of alive neighbors in the surroundings: " + count(board, 2, 2));
        System.out.println("CellValue at next gen: " + cellValue(board, 2, 2));
	}
	...
	public static int[][] read(String fileName) {
		...
		int[][] board = new int[rows + 2][cols + 2]; // the +2 is for the "extreme cases" to be able to check their surroundings as well.
		boolean noMoreLines = false;
		String checkNextRow = "";
		int startingRow = 1;
		while (!noMoreLines) {
			checkNextRow = in.readLine(); 
			if (checkNextRow == null) {
				noMoreLines = true;
			}
			else{
	...
	public static int cellValue(int[][] board, int i, int j) {
		int surroundingcell = count(board, i, j);  // representing the value of up to 8 slots around the chosen cell ( only considering those alive)
		int cellValue = board[i][j];
		if (cellValue == 1){ // rules number 1, 2 and 3
		 if (surroundingcell == 2 || surroundingcell == 3) {
			return 1;
			}
			else {
				return 0;
	...
	public static int count(int[][] board, int i, int j) {
		...
		for (int row = i - 1; row <= i +1; row++  ) {   
			for(int col = j -1; col <= j + 1; col++){
				if (board[row][col] == 1 && (i != row || j != col)) {
					count++;
